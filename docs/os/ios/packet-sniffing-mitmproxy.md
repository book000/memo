...
- 設定も面倒。Fiddler 内の色んな設定をいじらないと期待する動作をしないので、直感的ではなかった。
...
mitmproxy では「アドオン」という機能を用いることで、Python スクリプトで処理をインジェクトできる。
...
Gist に置いた [mitmproxy-all-write-file.py](https://gist.github.com/book000/ae5685c7903325f76ffefee115ca67e8) を使って以下のように実行する。  
