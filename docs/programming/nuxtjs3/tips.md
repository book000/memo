...
ソースがどこだったか忘れたのだが、Vue 3 では以下の順番で各種タグブロックを置くのが一般的のようだ。
...
Vue は script タグ以下の書き方がいろいろありすぎて困るのだが、[Vue の公式ドキュメント](https://v3.ja.vuejs.org/api/sfc-script-setup.html) で `<script setup>` の書き方がお勧めと言われているので、とりあえずこれを使うことにする。

`<script setup>` では以下の特徴があるようだ。

- コンポーネントインスタンスが作成されるたびに実行（`oncreated` イベントが不要になる）
- 変数・関数宣言・インポートなどは `<template>` 内で直接利用可能
- 従来、`data()` 内で定義していた変数は `const count = ref(0)` のように定義するようだ。
- 従来、`props()` 内で定義していたプロパティ的なアレは、`defineProps` 関数を用いて定義するようだ。
- `emit` は `defineEmits` 関数で定義したうえで、`emit('hoge')` のように呼び出せるようだ。
- 従来、`method()` 内で定義していたメソッドはコンポーネントの呼び出し元から `$ref` 経由で呼び出すことができた。Vue 3 からは外部から呼び出しても良いメソッドを明示的に `defineExpose` で公開しなければならないようだ。

など、これらに沿って書くと以下のようなものが作れる。（長いので折りたたんでいる）
